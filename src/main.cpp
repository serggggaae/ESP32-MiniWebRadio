...
void ir_short_key(uint8_t key) {
    ...
    switch(key) {
        ...
        case 13:    // ARROW DOWN
                    ...
                    break;
        case 11:    // ARROW RIGHT
                    if(_state == STATIONSLIST) {lst_RADIO.nextPage(); setTimeCounter(20); break;}  // next page
		...
		case 19:    // CHANGE SUBMENUE
					if(_state == BLUETOOTH) {changeState(BRIGHTNESS); break;}
					...
					if(_radioSubmenue == 2) _radioSubmenue = -1; 
					connecttohost(_settings.lastconnectedhost);
					break;
			default:    break;
    }
...
void ir_long_key(int8_t key) {
    ...
    if(key == 20) fall_asleep(); // LONG SLEEP
	if(key == 21) { // LONG CANCEL
				  if(_state == RADIO) {_radioSubmenue = 0; stopSong(); changeState(RADIO);}
